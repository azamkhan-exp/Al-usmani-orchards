...
export async function POST(req: NextRequest) {
  try {
    ...
    await runTransaction(async (database: any) => {
      ...
      await database.prepare('UPDATE password_reset_tokens SET used = 1 WHERE id = ?').run(record.id);

      // 3. Invalidate old sessions for security
      await database.prepare('DELETE FROM user_sessions WHERE user_id = ?').run(record.user_id);
    });
